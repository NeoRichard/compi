...
import coolc.parser.*;
import coolc.symtable.SemanticCopy;
...
public class SemanticCopy {

	static SymTable t;     

    

    public  SemanticCopy(Program root) { 
    	t = new SymTable(root);     
    	print(root);
...
//        System.out.println();
    ...
    private static void print(Feature f, String currentType) {
...
//            System.out.printf("field %s %s\n", var.getType(), var.getId());
            ...
            if( var.getValue() != null ) {
                print(var.getValue(), 3, currentType);
            }
        }
        else {
    ...
    private static void print(Expr e, int indent, String currentType) {
    ...
    //        System.out.println("block");
            for(Expr child: ((Block)e).getStatements()) {
                print(child, indent+1, currentType);
            }
        }
    ...
    //        System.out.println("while");
            print(loop.getCond(), indent+1, currentType);

            printIndent(indent);
    //        System.out.println("loop");
            print(loop.getValue(), indent+1, currentType);


        }
        else if(e instanceof AssignExpr) {
//            System.out.printf("assign %s\n", ((AssignExpr)e).getId());
            print(((AssignExpr)e).getValue(), indent+1, currentType);
        }        
...
//            System.out.printf("call %s", call.getName());
    ...
    //        System.out.println();
        ...
        //        System.out.println("callee");
                print(call.getExpr(), indent+2, currentType);
            }
            if (call.getArgs().size() > 0) {
                printIndent(indent+1);
        //        System.out.println("args");
                for(Expr arg: call.getArgs()) {
                    print(arg, indent+2, currentType);
                }
            }
        }
    ...
    //        System.out.println("if");
            print(cond.getCond(), indent+1, currentType);

            printIndent(indent);
    //        System.out.println("then");
            print(cond.getTrue(), indent+1, currentType);

            printIndent(indent);
    //        System.out.println("else");
            print(cond.getFalse(), indent+1, currentType);
...
//            System.out.printf("unary %s\n", operator(expr.getOp()));
            System.out.println("UnaryExpr: "+expr.getValue());
//        	currentType = type;
            print(expr.getValue(), indent + 1, currentType);
        }
        else if(e instanceof BinaryExpr) {
            System.out.println("BinaryExpr");
            BinaryExpr expr = (BinaryExpr)e;
//            System.out.printf("binary %s\n", operator(expr.getOp()));
            print(expr.getLeft(), indent + 1, currentType);   
            print(expr.getRight(), indent + 1, currentType);
            }
        else if (e instanceof CaseExpr) {
            System.out.println("CaseExpr");
            CaseExpr caseExpr = ((CaseExpr)e);
    //        System.out.println("instanceof");
            print(caseExpr.getValue(), indent+1, currentType);
    ...
    //            System.out.printf("case %s %s\n", c.getType(), c.getId());
                print(c.getValue(), indent+2, currentType);
            }
    ...
    //            System.out.printf("%s %s\n", var.getType(), var.getId());
                ...
                if(var.getValue() != null) {
                    print(var.getValue(), indent+3, currentType);
                }
            }

            print(let.getValue(), indent+1, currentType);
        }
