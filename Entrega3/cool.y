...
class
    ...
    | "class" TYPE[type] "inherits" TYPE[super] "{" class_body "}"    
            { $$ = new ClassDef($type, $super, $class_body); }
    ;
...
var_declaration
    : ID ":" TYPE 
            {  $$ = new Variable($ID, $TYPE); }
    | ID ":" TYPE "<-" expr
            {  $$ = new Variable($ID, $TYPE, $expr); }
    ;
...
expr 
    ...
    | expr[obj] "@" TYPE "." ID "(" args  ")" 
            { $$ = new DispatchExpr($obj, $TYPE, $ID, $args); }

    | expr[obj] "." ID "(" args  ")"
            { $$ = new DispatchExpr($obj, $ID, $args); }

    | ID "(" args ")"  
            { $$ = new DispatchExpr($ID, $args ); }

    | "if" expr[cond] "then" expr[true] "else" expr[false] "fi"
            { $$ = new IfExpr($cond, $true, $false); }

    | "while" expr[cond] "loop" expr[stmt] "pool"
            { $$ = new WhileExpr($cond, $stmt); }

    | "{" stmt_list "}"
            { $$ = new Block($stmt_list); }

    | "let" let_list "in" expr[value]
            { $$ = new LetExpr($let_list, $value); }

    | "case" expr[value] "of" case_list "esac" 
            { $$ = new CaseExpr($value, $case_list); }

    | "new" TYPE        { $$ = new NewExpr($TYPE); }
